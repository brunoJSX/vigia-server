...
func TestBuildDailySummary_ConsolidatesPreviousDayPerActiveMonitor(t *testing.T) {
	...
	active := monitor.New("mon-active", "https://example.com", monitor.TypeUptime, 3, time.Minute, 5*time.Second)
	if err := monitors.Save(ctx, active); err != nil {
		t.Fatalf("setup: %v", err)
	}

	paused := monitor.New("mon-paused", "https://example.org", monitor.TypeUptime, 3, time.Minute, 5*time.Second)
	paused.Pause()
